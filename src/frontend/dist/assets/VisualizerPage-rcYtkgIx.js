import { c as createLucideIcon, a as commonjsGlobal, g as getDefaultExportFromCjs, r as reactExports, b as clsx, R as React2, j as jsxRuntimeExports, S as Slot, d as cn, e as cva, f as React, h as composeRefs, u as useComposedRefs } from "./index-uty3N76U.js";
/**
...
function CustomTooltip({ active, payload, label }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-card border border-border rounded-lg px-3 py-2 shadow-elevation text-xs font-mono", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-muted-foreground", children: [
      "Iter ",
      label
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-accent font-semibold", children: [
      "Loss: ",
...
function ConvergenceChart({
  ...
  var _a, _b;
  const displayData = lossHistory.length > 100 ? lossHistory.filter(
    (_, i) => i % Math.ceil(lossHistory.length / 100) === 0 || i === lossHistory.length - 1
  ) : lossHistory;
  const lineColor = convergenceStatus === "converged" ? "oklch(0.68 0.22 145)" : convergenceStatus === "diverging" ? "oklch(0.55 0.22 25)" : "oklch(0.72 0.2 265)";
  const lastLoss = ((_a = lossHistory[lossHistory.length - 1]) == null ? void 0 : _a.loss) ?? 1;
  const yMax = Math.max(lastLoss * 1.1, 0.05);
  const firstLoss = ((_b = lossHistory[0]) == null ? void 0 : _b.loss) ?? 1;
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full h-full flex flex-col", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: "100%", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    LineChart,
    {
      data: displayData,
...
function lossFunction(w1, w2) {
  return 0.5 * (w1 * w1 + 2 * w2 * w2) + 0.1 * Math.sin(3 * w1) * Math.cos(2 * w2);
}
function gradW1(w1, w2) {
  return w1 + 0.3 * Math.cos(3 * w1) * Math.cos(2 * w2);
}
function gradW2(w1, w2) {
...
function computeLossSurface() {
  ...
  for (let i = 0; i < GRID_SIZE; i++) {
    for (let j = 0; j < GRID_SIZE; j++) {
      const w1 = -W_RANGE$1 + i / (GRID_SIZE - 1) * 2 * W_RANGE$1;
      const w2 = -W_RANGE$1 + j / (GRID_SIZE - 1) * 2 * W_RANGE$1;
...
function makeInitialState(w1, w2) {
...
function useSimulation() {
  ...
  const performStep = reactExports.useCallback(
    (currentState) => {
      var _a;
      ...
      const clampedW1 = Math.max(
        -W_RANGE$1 * 1.5,
        Math.min(W_RANGE$1 * 1.5, newW1)
      );
      const clampedW2 = Math.max(
        -W_RANGE$1 * 1.5,
        Math.min(W_RANGE$1 * 1.5, newW2)
      );
      ...
      else if (newLoss > ((_a = lossHistory[lossHistory.length - 1]) == null ? void 0 : _a.loss) * 2 && iteration > 5)
        status = "diverging";
      return {
        ...currentState,
        w1: clampedW1,
  ...
  const setLearningRate = reactExports.useCallback((lr) => {
    setState((prev) => ({ ...prev, learningRate: lr }));
  }, []);
  const setInitialWeights = reactExports.useCallback(
    (w1, w2) => {
      pause();
  ...
  reactExports.useEffect(() => {
    if (state.isRunning && state.convergenceStatus !== "converged" && state.convergenceStatus !== "diverging") {
      intervalRef.current = setInterval(() => {
        setState((prev) => {
          if (!prev.isRunning) return prev;
          const next = performStep(prev);
          if (next.convergenceStatus === "converged" || next.convergenceStatus === "diverging") {
...
const W_RANGE = 2;
function lossToColor(normalizedLoss) {
  const t2 = Math.max(0, Math.min(1, normalizedLoss));
  if (t2 < 0.25) {
    const s22 = t2 / 0.25;
    return [
      Math.round(30 + s22 * 20),
      Math.round(30 + s22 * 40),
      Math.round(160 + s22 * 70)
    ];
  }
  if (t2 < 0.5) {
    const s22 = (t2 - 0.25) / 0.25;
    return [
      Math.round(50 + s22 * 20),
      Math.round(70 + s22 * 100),
      Math.round(230 - s22 * 80)
    ];
  }
  if (t2 < 0.75) {
    const s22 = (t2 - 0.5) / 0.25;
    return [
      Math.round(70 + s22 * 160),
      Math.round(170 - s22 * 70),
      Math.round(150 - s22 * 100)
    ];
  }
  const s2 = (t2 - 0.75) / 0.25;
  return [
    Math.round(230 + s2 * 20),
    Math.round(100 - s2 * 60),
    Math.round(50 - s2 * 30)
  ];
}
function LossSurfaceCanvas({
  lossSurface,
  w1,
  w2,
  gradW1Dir,
  ...
  const draw = reactExports.useCallback(() => {
    ...
    if (!ctx) return;
    ctx.scale(dpr, dpr);
    const pad2 = { top: 10, right: 10, bottom: 32, left: 36 };
    const plotW = width - pad2.left - pad2.right;
    const plotH = height - pad2.top - pad2.bottom;
    ctx.clearRect(0, 0, width, height);
    const losses = lossSurface.map((p2) => p2.loss);
    ...
    for (let i = 0; i < GRID_SIZE; i++) {
      for (let j = 0; j < GRID_SIZE; j++) {
        ...
        const [r, g2, b2] = lossToColor(norm);
        ctx.fillStyle = `rgb(${r},${g2},${b2})`;
        const x2 = pad2.left + i * cellW;
        const y2 = pad2.top + (GRID_SIZE - 1 - j) * cellH;
        ctx.fillRect(x2, y2, Math.ceil(cellW) + 1, Math.ceil(cellH) + 1);
      }
    }
    const toCanvasX = (wv) => pad2.left + (wv + W_RANGE) / (2 * W_RANGE) * plotW;
    const toCanvasY = (wv) => pad2.top + (1 - (wv + W_RANGE) / (2 * W_RANGE)) * plotH;
    if (posHistory.length > 1) {
      ...
      for (let k2 = 1; k2 < posHistory.length; k2++) {
        const pt = posHistory[k2];
        ctx.lineTo(toCanvasX(pt.w1), toCanvasY(pt.w2));
      }
      ctx.stroke();
      ctx.setLineDash([]);
      posHistory.forEach((pt, k2) => {
        if (k2 % 5 !== 0 && k2 !== posHistory.length - 1) return;
        const cx2 = toCanvasX(pt.w1);
        const cy2 = toCanvasY(pt.w2);
        const progress = k2 / (posHistory.length - 1);
        const alpha3 = 0.3 + progress * 0.5;
        ctx.beginPath();
        ctx.arc(cx2, cy2, 3, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(100, 200, 255, ${alpha3})`;
        ctx.fill();
      });
      ctx.restore();
    }
    ...
    for (const v2 of [-2, -1, 0, 1, 2]) {
      const x2 = toCanvasX(v2);
      ctx.fillText(String(v2), x2, height - 4);
      ctx.fillStyle = "rgba(255,255,255,0.08)";
      ctx.beginPath();
      ctx.moveTo(x2, pad2.top);
      ctx.lineTo(x2, pad2.top + plotH);
      ctx.stroke();
      ctx.fillStyle = "rgba(200, 200, 210, 0.7)";
    }
    ctx.textAlign = "right";
    for (const v2 of [-2, -1, 0, 1, 2]) {
      const y2 = toCanvasY(v2);
      ctx.fillText(String(v2), pad2.left - 4, y2 + 4);
    }
    ctx.fillStyle = "rgba(200, 200, 210, 0.5)";
    ctx.font = "11px sans-serif";
    ctx.textAlign = "center";
    ctx.fillText("w₁", pad2.left + plotW / 2, height - 2);
    ctx.save();
    ctx.translate(10, pad2.top + plotH / 2);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: containerRef, className: "w-full h-full min-h-[300px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { ref: canvasRef, className: "w-full h-full" }) });
}
function MetricRow({
  label,
  value,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between py-1.5 border-b border-border/40 last:border-0", children: [
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-3", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        className: `flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-display font-semibold transition-smooth ${isConverged ? "bg-accent/15 border-accent/40 text-accent" : isDiverging ? "bg-destructive/15 border-destructive/40 text-destructive" : convergenceStatus === "running" ? "bg-primary/10 border-primary/30 text-primary" : "bg-muted/50 border-border text-muted-foreground"}`,
        children: [
          isConverged ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheck, { className: "w-4 h-4" }) : isDiverging ? /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "w-4 h-4" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Activity, { className: "w-4 h-4" }),
          isConverged ? "Converged! Loss < 0.01" : isDiverging ? "Diverging — reduce learning rate" : convergenceStatus === "running" ? "Descending…" : "Ready to run"
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-card/60 border border-border rounded-lg px-3 py-1", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        MetricRow,
        {
          label: "Current Loss",
          ...
          highlight: isConverged ? "accent" : loss < 0.1 ? "primary" : void 0
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        MetricRow,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-muted-foreground leading-snug", children: "The gradient magnitude tells us how steep the slope is. When it approaches zero, the weights have found a minimum of the loss surface." })
  ] });
}
...
const LR_MAX = 1;
const LR_STEPS = 1e3;
function sliderToLR(v2) {
  const logMin = Math.log10(LR_MIN);
  const logMax = Math.log10(LR_MAX);
  return 10 ** (logMin + v2 / LR_STEPS * (logMax - logMin));
}
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "span",
        {
          ...
          children: [
            "Iteration:",
            " ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-foreground font-semibold", children: iteration })
          ]
        }
      )
    ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between text-xs", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "span",
          {
            ...
            children: learningRate.toFixed(4)
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Slider,
        {
          ...
          max: LR_STEPS,
          step: 1,
          onValueChange: ([v2]) => onLearningRateChange(sliderToLR(v2)),
          className: "w-full"
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between text-[10px] text-muted-foreground font-mono", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: LR_MIN }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "0.01" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "0.1" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: LR_MAX })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-muted-foreground leading-snug", children: "Controls how large each weight update step is. Too high → diverges. Too low → slow convergence." })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "label",
        {
          ...
          children: "Starting Position (w₁, w₂)"
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-mono text-muted-foreground w-5", children: "w₁" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Slider,
          {
            id: "w1-slider",
            ...
            max: 2,
            step: 0.1,
            onValueChange: ([v2]) => onInitialWeightsChange(v2, initialW2),
            className: "flex-1"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-mono text-muted-foreground w-10 text-right", children: initialW1.toFixed(1) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-mono text-muted-foreground w-5", children: "w₂" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Slider,
          {
            ...
            max: 2,
            step: 0.1,
            onValueChange: ([v2]) => onInitialWeightsChange(initialW1, v2),
            className: "flex-1"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-mono text-muted-foreground w-10 text-right", children: initialW2.toFixed(1) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-muted-foreground leading-snug", children: "Initial weight values — the starting point on the loss surface. Different starts may find different paths to the minimum." })
    ] })
  ] });
}
function VisualizerPage() {
  ...
  const posHistoryRef = reactExports.useRef([{ w1, w2 }]);
  const lastIterRef = reactExports.useRef(iteration);
  reactExports.useEffect(() => {
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-display font-bold text-xl md:text-2xl text-foreground leading-tight", children: "Gradient Descent Visualization" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-body", children: "Watch how gradient descent navigates the loss surface to find minimum error in a 2-parameter model." })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-3 md:gap-4 min-h-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pt-3 pb-2 border-b border-border/50 bg-card", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-display font-semibold text-sm text-foreground", children: "2D Loss Surface Visualization" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-muted-foreground mt-0.5", children: "Heatmap of loss(w₁, w₂) — warm colors = high loss, cool = low. White dot shows current weights." })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 p-2 min-h-[280px] md:min-h-[360px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                  LossSurfaceCanvas,
                  {
                    lossSurface,
                    w1,
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 pb-3 pt-1", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-[10px] text-muted-foreground font-body leading-relaxed", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[oklch(0.75_0.18_85)] font-semibold", children: "Yellow arrow" }),
                  " ",
                  "= gradient direction (steepest ascent). Descent moves opposite.",
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-[oklch(0.72_0.13_235)] font-semibold", children: [
                    " ",
                    "Dashed path"
                  ] }),
                  " ",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-3 md:gap-4 min-h-0", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                className: "bg-card border border-border rounded-xl overflow-hidden flex flex-col shadow-elevation",
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pt-3 pb-2 border-b border-border/50", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-display font-semibold text-sm text-foreground", children: "Convergence Chart" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-muted-foreground mt-0.5", children: "Loss vs. iteration — the curve should descend and flatten as the model learns." })
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 min-h-[180px] md:min-h-[200px] p-2 pb-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                    ConvergenceChart,
                    {
                      lossHistory,
                      convergenceStatus
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-display font-semibold text-sm text-foreground mb-3", children: "Simulation Controls" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    SimulationControls,
                    {
                      isRunning,
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                className: "bg-card border border-border rounded-xl p-4 shadow-elevation",
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-display font-semibold text-sm text-foreground mb-3", children: "Live Metrics" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    MetricsPanel,
                    {
                      loss,
